    ...
    for row in rows:
        ...
        if not (prevrow is None) and prevrow['channelindex'] == row['channelindex'] and prevrow['antenna'] == row['antenna'] and prevrow['epc96'] == row['epc96']:
            if not ((4 * (1.0 / timescale) * (float(row['relative_timestamp']) - float(prevrow['relative_timestamp'])) * math.pi) == 0):
                deltaphase = (float(row['phase_rads']) -
                              float(prevrow['phase_rads']))
                if deltaphase >= 2 * math.pi:
                    deltaphase = deltaphase - 2 * math.pi
                if deltaphase < -1 * math.pi:
                    deltaphase = deltaphase + 2 * math.pi
                velocity_by_phase = (3e8 * 1.0 / freqbychannel(int(row['channelindex']))) * deltaphase * 1.0 / (4 * (
                    1.0 / timescale) * (float(row['relative_timestamp']) - float(prevrow['relative_timestamp'])) * math.pi)
            ...
            if not ((4 * (1/timescale) * (float(row['relative_timestamp']) - float(prevreads[row['channelindex'] + '|' + row['antenna'] + '|' + row['epc96']]['relative_timestamp'])) * math.pi) == 0):
                deltaphase = (float(row['phase_rads']) - float(
                    prevreads[row['channelindex'] + '|' + row['antenna'] + '|' + row['epc96']]['phase_rads']))
                if deltaphase >= 2 * math.pi:
                    deltaphase = deltaphase - 2 * math.pi
                if deltaphase < -1 * math.pi:
                    deltaphase = deltaphase + 2 * math.pi
                velocity_by_phase = (cspeed * 1.0 / freqbychannel(int(row['channelindex']))) * deltaphase * 1.0 / (4 * (1/timescale) * (float(
                    row['relative_timestamp']) - float(prevreads[row['channelindex'] + '|' + row['antenna'] + '|' + row['epc96']]['relative_timestamp'])) * math.pi)
                doppler_by_phase = velocity_by_phase * \
                    freqbychannel(int(row['channelindex'])) * 1.0 / 3e8
        else:
            velocity_by_phase = np.nan
        ...
        prxLinear = 10**(float(row['rssi']) * 0.1) * \
            1000  # convert to Watts from dbm
        ...
        wavelength = (
            cspeed * 1.0 / freqbychannel(int(row['channelindex'])))  # lambda
        #prx_moving_parts = ((1.0/prxLinear) * (4 * math.pi)**4) * 1.0 / (ptx * gr**2 * wavelength**4)
        # using aperature instead of gain for gt and gr terms, with the aperature formula integrated into prx_moving_parts
        prx_moving_parts = (ptx * gr**2) * 1.0 / (prxLinear * (wavelength**4))
        ...
        prxval = float(row['prx_moving_parts']) + \
            (xi * (50-int(row['channelindex'])))
