...
def usage(flask_port, flask_host, do_debug, db_path, flush, key_path_prefix, dispatchsleep):
    ...
    print('where <options> are:\n' \
        ...
        '\t-v - Enable VarIOT instead of sqlite (also add -t xxx for the API token, -b for the hostname of the API endpoint, -c xxx for the device ID, -z for the comma-separated telemetry fields, and -y for the personal token)' \
...
def getopts():
    ...
    # Check command line
    optlist, list = getopt.getopt(sys.argv[1:], 'hp:f:db:k:mos:w:e:lrt:vc:z:y:')
    for opt in optlist:
        if opt[0] == '-h':
        ...
        if opt[0] == '-v':
            variot = True
        if opt[0] == '-z':
            telemetryfields = opt[1]
        if opt[0] == '-y':
            personaltoken = opt[1]
            
    if do_debug:
        print('Parameters: [flask host = %s, flask port = %d, debug = %s, database = %s, key = %s, mysql = %d, mongo = %d, db_user = %s, db_password = %s, flush = %s, dispatchsleep = %s, redcap = %s, variot = %s, token = %s, device = %s, telemetryfields = %s, personaltoken = %s]' % (
            flask_host, flask_port, do_debug, db_path, key_path_prefix, mysql, tinymongo, db_user, db_password, flush, dispatchsleep, redcap, variot, token, device, telemetryfields, personaltoken))

    return flask_port, flask_host, do_debug, db_path, key_path_prefix, mysql, tinymongo, db_user, db_password, flush, dispatchsleep, redcap, variot, token, device, telemetryfields, personaltoken
...
if __name__ == '__main__':
    ...
    if redcap == True:
        ...
        database = VarIOTDatabase(crypto=crypto, db_path=db_path, dispatchsleep=dispatchsleep, token=token, device=device, telemetryfields=telemetryfields, personaltoken=personaltoken)
    else:
