...
def getopts():
    ...
    for opt in optlist:
        if opt[0] == '-h':
            usage(flask_host, sqlitedb_path, key_path_prefix, password, mysqldb_path, db_user, db_password)
        if opt[0] == '-p':
            password = opt[1]
        if opt[0] == '-f':
            flask_host = opt[1]
...
def main():
    ...
    mysqldb = MysqlDatabase(crypto=crypto, db_path=mysqldb_path, db_password=db_password, db_user=db_user)
    
    data = mysqldb.fetch_all(password) # this is sorted by interrogator_timestamp
    
    myjson = data # assumed to be a json array
    
    rows = 0
    db_runs = 0 # keep track of how many database runs you find (each time the relative time reverts to 0)
    ...
    for row in myjson:
        ...
        if relative_time == 0 or rows == 0 or sqlitedb is None:
            ...
            sqlitedb = SqliteDatabase(crypto=crypto, db_path=sqlitedb_path + str(db_runs))
            
        rows = rows + 1
        
        #print 'Adding row', rssi, relative_time, interrogator_time, rssi, epc96, doppler, phase, antenna, 'with password', db_password
        sqlitedb.insert_row(relative_time, interrogator_time, rssi, epc96, doppler, phase, antenna, rospecid, channelindex, tagseencount, accessspecid, inventoryparameterspecid, lastseentimestamp, db_password)        
