...
def augment(data, timescale=1e6):
    ...
    for row in rows:
        ...
        if not (prevrow is None) and prevrow['channelindex'] == row['channelindex'] and prevrow['antenna'] == row['antenna'] and prevrow['epc96'] == row['epc96']:
            try:
                velocity_by_phase = (3e8 * 1.0 / freqbychannel(int(row['channelindex']))) * (float(row['phase_rads']) - float(prevrow['phase_rads'])) * 1.0 / (4 * (1.0 / timescale) * (float(row['relative_timestamp']) - float(prevrow['relative_timestamp'])) * math.pi)
            ...
            if not ((4 * (1/timescale) * (float(row['relative_timestamp']) - float(prevreads[row['channelindex'] + '|' + row['antenna'] + '|' +  row['epc96']]['relative_timestamp'])) * math.pi) == 0):
                velocity_by_phase = (cspeed * 1.0 / freqbychannel(int(row['channelindex']))) * (float(row['phase_rads']) - float(prevreads[row['channelindex'] + '|' + row['antenna'] + '|' + row['epc96']]['phase_rads'])) * 1.0 / (4 * (1/timescale) * (float(row['relative_timestamp']) - float(prevreads[row['channelindex'] + '|' + row['antenna'] + '|' + row['epc96']]['relative_timestamp'])) * math.pi)
                doppler_by_phase = velocity_by_phase * freqbychannel(int(row['channelindex'])) * 1.0 / 3e8
        else:
            velocity_by_phase = np.nan
            doppler_by_phase = np.nan
        ...
        row['prx_moving_parts_deoscillated'] = float(row['prx_moving_parts']) + (0.009405417 * (50-int(row['channelindex'])))
...
def main():
    ...
    overlap_factor = 4
    freqs, times, Sxx = scipy.signal.spectrogram(data, fs=Fs, nperseg=framelength, noverlap=int(framelength * (1-1.0/overlap_factor)), detrend='linear')
    freqs_phase, times_phase, Sxx_phase = scipy.signal.spectrogram(data, fs=Fs, nperseg=framelength, noverlap=int(framelength * (1-1.0/overlap_factor)), detrend='linear', mode='phase')
    freqs_angle, times_angle, Sxx_angle = scipy.signal.spectrogram(data, fs=Fs, nperseg=framelength, noverlap=int(framelength * (1-1.0/overlap_factor)), detrend='linear', mode='angle')     
    ...
    for i in range(len(times)):
        ...
        for j in range(len(Sxx)):
            ...
            row['start_time'] = center_realtive_timestamp - (window_size * timescale * 1.0 / 2)
            row['end_time'] = center_realtive_timestamp + (window_size * timescale * 1.0 / 2)
