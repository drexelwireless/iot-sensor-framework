...
class REDCapDatabase(Database):
    ...
    def redcap_dispatch(self, recordsdictlist):                
        ...
        for record in recordsdictlist:
            ...
            record['rssi'] = self.db_encrypt(record['rssi'], record['interrogatortime'])
            record['doppler'] = self.db_encrypt(record['doppler'], record['interrogatortime'])
            record['phase'] = self.db_encrypt(record['phase'], record['interrogatortime'])
            record['epc96'] = self.db_encrypt(record['epc96'], record['interrogatortime'])
