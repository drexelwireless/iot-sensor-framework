...
def ws_web_stats_relativetimewindows(start, end, time=4):
    ...
    data = ws_database.fetch_between_window(start, end, db_pw)
    data = ws_database.break_into_timewindows(data, time, 'relative_timestamp', 'rssi')
...
def ws_web_stats_windows(start, end, size=128):
    ...
    data = ws_database.fetch_between_window(start, end, db_pw)
    data = ws_database.break_into_windows(data, size, 'relative_timestamp', 'rssi')
...
def ws_add_data():
    ...
    for row in insert_list:         
        ...
        interrogator_time = getwithdefault(row['data'], 'interrogator_time', "-1")
        ...
        inventoryparameterspecid = getwithdefault(row['data'], 'inventoryparameterspecid', "-1")
        lastseentimestamp = getwithdefault(row['data'], 'lastseentimestamp', "-1")

        relative_time = int(relative_time)
        interrogator_time = int(interrogator_time)
                
        ws_database.insert_row(relative_time, interrogator_time, rssi, epc96, doppler, phase, antenna, rospecid, channelindex, tagseencount, accessspecid, inventoryparameterspecid, lastseentimestamp, db_pw = db_pw)
...
def ws_start(crypto, database, flask_host='0.0.0.0', flask_port=5000, do_debug=False, use_reloader=False):
    ...
    app.run(debug = ws_do_debug, port = ws_flask_port, host = ws_flask_host, ssl_context=context, threaded=False, use_reloader=use_reloader) #multithreaded web server cannot share database connection
