...
def getopts():
    ...
    for opt in optlist:
        if opt[0] == '-h':
            usage(flask_port, flask_host, do_debug, db_path, flush, key_path_prefix, dispatchsleep)
        if opt[0] == '-p':
    ...
    if do_debug:
        print 'Parameters: [flask host = %s, flask port = %d, debug = %s, database = %s, key = %s, mysql = %d, db_user = %s, db_password = %s, flush = %s, dispatchsleep = %s, redcap = %s, redcap_token = %s]' % (flask_host, flask_port, do_debug, db_path, key_path_prefix, mysql, db_user, db_password, flush, dispatchsleep, redcap, redcap_token)
...
def start_wserver(crypto, database, flask_host, flask_port, do_debug):
    ws_start(crypto, database, flask_host=flask_host, flask_port=flask_port, do_debug=do_debug)
...
if __name__ == '__main__':
    ...
    if redcap == True:
        database = REDCapDatabase(crypto=crypto, db_path=db_path, token=redcap_token, dispatchsleep=dispatchsleep)
    elif mysql == True:
        database = MysqlDatabase(crypto=crypto, db_path=db_path, db_password=db_password, db_user=db_user, flush=flush, dispatchsleep=dispatchsleep)
    else:
        database = SqliteDatabase(crypto=crypto, db_path=db_path, flush=flush, dispatchsleep=dispatchsleep)
