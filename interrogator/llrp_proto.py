...
# *** modified by Bill Mongan 11/14/13
...
# Copyright (C) 2013 William M. Mongan <billmongan@gmail.com>
#
...
def recv_message(connection):
    ...
    # mongan
    logger.debug("BEGIN PRE UNPACK")
    logger.debug(data)
    logger.debug("END PRE UNPACK")

    # mongan sometimes data is empty! don't try to unpack; return empty msg
    if len(data) == 0:
        return msg
    ...
    if (ver != VER_PROTO_V1):
        raise LLRPError('messages version %d are not supported, need ver %d from type %d' % (
            ver, VER_PROTO_V1, type))  # mongan
    ...
    msg[name]['Type'] = type
    msg[name]['ID'] = id
    # mongan BEGIN/END debug messages throughout for tracing
    logger.debug('BEGINRECEIVE')
    logger.debug(msg)
    logger.debug('ENDRECEIVE')
...
def decode_TagReportData(data):
    ...
    # mongan - also add to message struct, and add parameter struct and function
    ret, body = decode('Antenna')(body)
    ...
    if ret:
        ...
        if ret:
            ...
            raise LLRPError('missing or invalid Antenna parameter')

    ret, body = decode('RSSI')(body)
    ...
    if ret:
        ...
        if ret:
            ...
            raise LLRPError('missing or invalid RSSI parameter')

    ret, body = decode('Timestamp')(body)
...
Message_struct['TagReportData'] = {
    ...
    'fields': [
        'Type',
        'EPCData', 'EPC-96', 'Antenna', 'RSSI', 'Timestamp',  # mongan
        'ROSpecID',
    ],
...
# mongan antenna Parameter


def decode_Antenna(data):
    par = {}
    ...
    header = data[0: tve_header_len]
    (type, ), length = struct.unpack(tve_header, header), 1 + (16 / 8)
    type = type & BITMASK(3)
    if type != Message_struct['Antenna']['type']:
        return (None, data)
    body = data[tve_header_len: length]
    logger.debug('%s (type=%d len=%d)' % (func(), type, length))

    # Decode fields
    par['Antenna'] = body.encode('hex')

    return par, data[length:]


Message_struct['Antenna'] = {
    'type':	1,
    'fields': [
        'Type',
        'Antenna'
    ],
    'decode': decode_Antenna
}

# mongan RSSI Parameter
...
def decode_RSSI(data):
    ...
    header = data[0: tve_header_len]
    (type, ), length = struct.unpack(tve_header, header), 1 + (8 / 8)
    type = type & BITMASK(3)
    if type != Message_struct['RSSI']['type']:
        return (None, data)
    body = data[tve_header_len: length]
    logger.debug('%s (type=%d len=%d)' % (func(), type, length))

    # Decode fields
    par['RSSI'] = body.encode('hex')
...
# mongan Timestamp Parameter (set bitmask, size as N / 8, and type in struct)
...
def decode_Timestamp(data):
    ...
    header = data[0: tve_header_len]
    (type, ), length = struct.unpack(tve_header, header), 1 + (64 / 8)
    type = type & BITMASK(2)
    if type != Message_struct['Timestamp']['type']:
        return (None, data)
    body = data[tve_header_len: length]
    logger.debug('%s (type=%d len=%d)' % (func(), type, length))

    # Decode fields
    par['Timestamp'] = body.encode('hex')

    return par, data[length:]


Message_struct['Timestamp'] = {
    'type':	2,
    'fields': [
        'Type',
        'Timestamp'
...
def llrp_connect(connection, host, port=LLRP_PORT):
    ...
    try:
        status = ans['READER_EVENT_NOTIFICATION']['ReaderEventNotificationData']['ConnectionAttemptEvent']['Status']
    except:
        ...
        while True:
            ...
            while True:
                try:
                    ...
                    # mongan - specifically catch socket error and quit
                    #print "Socket done"
                    # return
                    break
                except:
                    # mongan added logger and e param, commented out all "junk at end of message" raise checks; instead of quitting, continue on error
                    e = sys.exc_info()
                    logger.debug('Error in msg receiver')
                    logger.debug(e[1])
                    traceback.print_exc()
                    # return
...
class LLRPdCapabilities(dict):
    ...
    def LLRPCapabilities(self, can_do_rfsurvey, can_report_buf_fill,
        ...
        self['LLRPdCapabilities']['LLRPCapabilities']['CanDoRFSurvey'] = can_do_rfsurvey
        self['LLRPdCapabilities']['LLRPCapabilities']['CanReportBufferFillWarning'] = can_report_buf_fill
        self['LLRPdCapabilities']['LLRPCapabilities']['SupportsClientRequestOpSpec'] = supports_client_reqs
        self['LLRPdCapabilities']['LLRPCapabilities']['CanDoTagInventoryStateAwareSingulation'] = can_do_tag_inv
        self['LLRPdCapabilities']['LLRPCapabilities']['SupportsEventAndReportHolding'] = supports_ev_rep_holding
        self['LLRPdCapabilities']['LLRPCapabilities']['MaxPriorityLevelSupported'] = max_prio
        self['LLRPdCapabilities']['LLRPCapabilities']['ClientRequestOpSpecTimeout'] = timeout
        self['LLRPdCapabilities']['LLRPCapabilities']['MaxNumROSpec'] = max_rospec
        self['LLRPdCapabilities']['LLRPCapabilities']['MaxNumSpecsPerROSpec'] = max_spec_x_rospec
        self['LLRPdCapabilities']['LLRPCapabilities']['MaxNumInventoryParametersSpecsPerAISpec'] = max_inv_x_aispec
        self['LLRPdCapabilities']['LLRPCapabilities']['MaxNumAccessSpec'] = max_accesspec
        self['LLRPdCapabilities']['LLRPCapabilities']['MaxNumOpSpecsPerAccessSpec'] = max_opspec_x_accesspec


class LLRPROSpec(dict):
    def __init__(self, id, priority=0, state='Disabled', ipspecid=0, ipprotoid=0):
        # Sanity checks
        if id <= 0:
        ...
        self['ROSpec']['ROBoundarySpec']['ROSpecStartTrigger'] = {}
        self['ROSpec']['ROBoundarySpec']['ROSpecStartTrigger']['ROSpecStartTriggerType'] = 'Null'

        self['ROSpec']['ROBoundarySpec']['ROSpecStopTrigger'] = {}
        self['ROSpec']['ROBoundarySpec']['ROSpecStopTrigger']['ROSpecStopTriggerType'] = 'Null'
        self['ROSpec']['ROBoundarySpec']['ROSpecStopTrigger']['DurationTriggerValue'] = 0

        self['ROSpec']['AISpec'] = {}
        self['ROSpec']['AISpec']['AntennaID'] = [0, ]

        self['ROSpec']['AISpec']['AISpecStopTrigger'] = {}
        self['ROSpec']['AISpec']['AISpecStopTrigger']['AISpecStopTriggerType'] = 'Null'
        self['ROSpec']['AISpec']['AISpecStopTrigger']['DurationTriggerValue'] = 0

        # mongan modified (both params were 0) per ADD_ROSPEC.llrp in llrp commander
        self['ROSpec']['AISpec']['InventoryParameterSpec'] = {}
        self['ROSpec']['AISpec']['InventoryParameterSpec']['InventoryParameterSpecID'] = ipspecid
        self['ROSpec']['AISpec']['InventoryParameterSpec']['ProtocolID'] = ipprotoid
