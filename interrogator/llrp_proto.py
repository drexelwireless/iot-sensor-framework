...
def recv_message(connection):
	...
	if (ver != VER_PROTO_V1) :
		raise LLRPError('messages version %d are not supported, need ver %d from type %d' % (ver, VER_PROTO_V1, type)) # mongan
...
class LLRPROSpec(dict):
	def __init__(self, id, priority = 0, state = 'Disabled', ipspecid = 0, ipprotoid = 0):
		...
		self['ROSpec']['AISpec']\
				['InventoryParameterSpec']\
				['InventoryParameterSpecID'] = ipspecid
		self['ROSpec']['AISpec']\
				['InventoryParameterSpec']\
				['ProtocolID'] = ipprotoid
