...
class LLRPClient(LineReceiver):
    ...
    def parseReaderConfig(self, confdict):
        ...
        conf = {}
        for k, v in list(confdict.items()):
    ...
    def get_tx_power(self, tx_power):
        ...
        ret = {}
        for antid, tx_power in list(tx_power.items()):
            if tx_power == 0:
    ...
    def setTxPower(self, tx_power):
        ...
        needs_update = False
        for ant, (tx_pow_idx, tx_pow_dbm) in list(tx_pow_validated.items()):
...
class LLRPClientFactory(ReconnectingClientFactory):
    ...
    def buildProtocol(self, addr):
        ...
        if hostport in self.antenna_dict:
            clargs['antennas'] = [
                int(x) for x in list(self.antenna_dict[hostport].keys())]
        elif addr.host in self.antenna_dict:
            clargs['antennas'] = [
                int(x) for x in list(self.antenna_dict[addr.host].keys())]
