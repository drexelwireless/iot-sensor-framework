...
class ImpinjR420(Interrogator):
    ...
    def start_server(self):
        ...
        self.fac = LLRPClientFactory(report_every_n_tags=1, # report every N>1 tags so that it reports more slowly to avoid lag; note this results in packets not sampled by the Impinj and only every 2 tags are reported, which it notes in TagSeenCount and the difference between FirstSeenTimestamp and LastSeenTimestamp; set PeriodicTriggerValue to something like 50 to group transmissions (will have to loop over messages received here), and set ROSpecStartTriggerType to 2 == Periodic
                                antennas=self.antennas, # 0 = all antennae but might not get configured by ROSpec unless explicitly enumerated
                                ...
                                periodictrigger=int(self.dispatchsleep * 1000), # convert to integer milliseconds
    ...
    def communication_consumer(self):
        ...
        while not self.exiting:
            ...
            resp, content = self.http_obj.request(uri=url, method='PUT', headers={'Content-Type':'application/json; charset=UTF-8'}, body=json.dumps(input_dicts))  
    ...
    def start(self):
        ...
        self.handler_thread = threading.Thread(target = self.handler_thread, args=())
        self.handler_thread.start()

        self.tag_dicts_queue = Queue.Queue()
        self.communication_thread = threading.Thread(target = self.communication_consumer, args=())
            ...
            for msg in input_msgs:
                ...
                for tag in tags:    
                    if 'FirstSeenTimestampUTC' in tag and 'EPC-96' in tag and 'AntennaID' in tag and 'PeakRSSI' in tag:
                        ...
                        self.out("Message did not contain all elements\n" + str(tag))
                        continue
                    ...
                    self.insert_tag(epc96, antenna, rssi, doppler, phase, first_seen_timestamp, rospecid, channelindex, tagseencount, lastseentimestamp, accessspecid, inventoryparameterspecid, self.start_timestamp)
    ...
    def insert_tag(self, epc, antenna, peak_rssi, doppler, phase, first_seen_timestamp, rospecid, channelindex, tagseencount, lastseentimestamp, accessspecid, inventoryparameterspecid, start_timestamp):
        ...
        self.out("Adding tag %s with RSSI %s and timestamp %s and ID %s on antenna %s with Phase %s and Doppler %s and Channel %s" % (str(self.count), str(peak_rssi), str(first_seen_timestamp), str(epc), str(antenna), str(doppler), str(phase), str(channelindex))) 
        
        input_dict = dict()
        input_dict['data'] = dict()
        input_dict['data']['db_password'] = self.db_password
        input_dict['data']['rssi'] = peak_rssi
        input_dict['data']['relative_time'] = first_seen_timestamp - start_timestamp
