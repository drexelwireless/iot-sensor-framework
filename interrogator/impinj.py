...
class Impinj(Interrogator):
    ...
    def start_server(self):
        ...
        try:
        	self.server = LLRPdConnection(self.ip_address, event_cb = self.handle_event)
    ...
    def start(self):
        ...
        self.client_thread = threading.Thread(target = self.start_server, args=())
                ...
                if msg.keys()[0] in repevents:
                    try: # sanity check that a message was received with a TagReportData
                        ...
                        if 'EPC-96' in innermsg: 
                            ...
                            epc = ''
                            self.out("WARNING: No EPC tag found in this message: " + msg)
                        antenna = innermsg['Antenna']['Antenna']
                        peak_rssi = int(innermsg['RSSI']['RSSI'], 16) # convert from hex byte
                        first_seen_timestamp = int(innermsg['Timestamp']['Timestamp'], 16) # convert from hex bytes
                        ...
                        self.insert_tag(epc, antenna, peak_rssi, first_seen_timestamp, self.start_timestamp)
                    except IndexError:
                        self.out("Ignoring message that contained no TagReportData")
    ...
    def insert_tag(self, epc, antenna, peak_rssi, first_seen_timestamp, start_timestamp):
        ...
        self.out("Adding tag %s with RSSI %s and timestamp %s and ID %s on antenna %s" % (str(self.count), str(peak_rssi), str(first_seen_timestamp), str(epc), str(antenna))) 
