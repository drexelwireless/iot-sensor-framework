    ...
    def insert_tag(self, tagarray):
        ...
        for entry in tagarray:
          ...
          for freeform in items:            
            timestamp = freeform['lastModifiedTime']
            epc = freeform['epc']
            # TODO Fill in x/y/z from RSSI here, and include RSSI/Phase/Doppler values as well, update client.py to import this module instead of localization once satisfied
            ...
            #self.out("Adding tag / collection %s with timestamp %s and epc %s and xPosition %s and yPosition %s and zPosition %s" % (str(self.count), str(timestampmicro), str(epc), str(xPos), str(yPos), str(zPos)))
